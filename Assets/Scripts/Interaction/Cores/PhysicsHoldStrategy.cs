...
public class PhysicsHoldStrategy : IHoldStrategy
{
    ...
    private const float POS_STRENGTH = 2000f;
    private const float ROT_STRENGTH = 100f;
    ...
    public void Hold(Rigidbody heldBody, Transform grabberTransform, Transform centerOfMassTransform)
    {
        ...
        Vector3 targetVelocity = positionDelta / dt; 
        
        float dist = positionDelta.magnitude;
        if (dist > 0.05f)
        {
            targetVelocity = Vector3.ClampMagnitude(targetVelocity, MAX_VELOCITY);
        }

        heldBody.linearVelocity = Vector3.Lerp(heldBody.linearVelocity, targetVelocity, 1f);
        ...
        if (Mathf.Abs(angle) > 0.01f)
        {
            ...
            heldBody.angularVelocity = Vector3.MoveTowards(heldBody.angularVelocity, angularTarget, 100f * dt);
        }
    }
}